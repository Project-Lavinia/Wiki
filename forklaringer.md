...
[1]: #stortingsvalg-med-sainte-lagu&#235;s-metode
[2]: #tabell-over-geografisk-fordeling
[3]: #geografisk-fordeling-av-distriktsmandater
[4]: #beregningsmetoder
[5]: #geografisk-fordeling-av-utjevningsmandater
[6]: #justert-restkvotient
[7]: #fordeling-av-utjevningsmandater-p\&#229;-partier
[8]: ./menu.md#restkvotienter
[9]: #stortingsvalg-med-d’Hondts-metode

Denne seksjonen forklarerer de tekniske delene av valgordningen.
...
Se [Tabell over geografisk fordeling][2]
...
Ta følgene regnestykker basert på tallene fra [tabellen ovenfor][2] med arealfaktor på 1.8. Regnestykkene illustrerer hvordan **fordelingstall** regnes ut.
...
Se [Geografisk fordeling av mandater][3]
...
<img src="https://render.githubusercontent.com/render/math?math=q=\frac{V}{2\cdot S %2B 1}">

hvor q er kvotienten, V er partienes andel av stemmene og S uttrykker antall mandater de har fått så langt i beregningen.

Se [Stortingsvalg med Sainte Laguës metode][1]
...
<img src="https://render.githubusercontent.com/render/math?math=q=\frac{V}{s %2B 1}">
...
Se [Stortingsvalg med d’Hondts metode][9]
...
[Stortingsvalg med Sainte-Laguës metode, steg 5:][1]  
   Parti A: 50 000 / 5   = 10000  
...
1. (14285 - 13333) = __952__ antall stemmer som skiller parti A og B.

2. (952 + 1) * 3 (delingstall) = __2 858__ antall _flere_ stemmer B trengte for å ta sistemandat.

3. 40000 + 2858 = __42 858__ minimum antall stemmer B trengte for å ta sistemandat (42858 / 3 = 14286).

## Restkvotient

I en valgkrets, etter at mandatene er delt ut, så er restkvotientene det samme som partiene sine gjenstående stemmeantall (etter at [Sainte-Laguës metode][4] er utført). Har partiet ikke vunnet noe mandat, er restkvotienten lik stemmetallet ved valget.1 Har partiet vunnet ett distriktsmandat, er restkvotienten lik stemmetallet dividert på 3.

Se eksemplet [Stortingsvalg med Sainte-Laguës metode, steg 5:][1]
...
Restkvotient kjent som _restbrøk_ og _siste kvoitent_.
...
<img src="https://render.githubusercontent.com/render/math?math=\text{justert restkvotient} = \frac{\text{partiet sin restkvotient}}{\text{gjennomsnittlig antall stemmer bak hvert mandat}}">
...
[Neste steg][5] er å avgjøre i hvilke fylker partiene skal få utjevningsmandatene sine.

_Merk at partier under sperregrensen (4 prosent oppslutning på landsbasis) er ikke med i konkurransen om utjevningsmandater._

## Geografisk fordeling av utjevningsmandater

For å fordele utjevningsmandatene på fylkene så må [justerte restkvotienter][6] regnes ut for partiene i hvert fylke. Dette regnestykket gjøres kun for partiene som skal ha utjevningsmandater.
...
Se [tabell over justerte restkvotienter I Lavinia][8].
